...
(parse/chunk/dedupe/curate/gate/promote)은 건드리지 않는다. 카테고리 1개당
...
# (slug, 카테고리명, 생성 가이드) — 사람이 직접 작성/리뷰한 리스트라
# LLM이 도메인을 임의로 정하지 않는다.
...
def build_basics_prompt(category_name: str, guide: str, n: int) -> str:
    """초급 학습자용 짧은 정의문 생성(build_prompt와 별도 프롬프트 — advanced
    출력에 영향 없음)."""
    return (
...
def build_intermediate_prompt(category_name: str, guide: str, n: int) -> str:
    """basics와 advanced 사이의 중간 난이도 — 독자는 정의는 알고 실무 선택/구성
    방법을 궁금해하는 엔지니어(build_basics_prompt/build_prompt와 별도 프롬프트,
    두 출력에 영향 없음)."""
    return (
