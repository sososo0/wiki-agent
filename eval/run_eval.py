...
호출로 함께 받아 qualitative_report를 만든다. promote.py가 정확히 "recall@k"/
"correctness" 키로 회귀를 판정하므로(HARD CONSTRAINT 경로), 이 옵트인 확장은
새 키만 추가한다 — qualitative=False(기본값)에서는 동작이 100% 동일하다.
...
def _parse_yes_no(text: str, context: str) -> int:
    """judge 응답을 yes=1/no=0으로 해석. 애매한 응답이면 경고 로그를 남기고
    여전히 0(fail-closed)으로 판정 — judge 캘리브레이션 문제를 가시화한다."""
...
def generate(query, hits, model=GEN_MODEL):
    ...
    API 호출 실패는 예외로 전파하지 않는다 — judge_fn이 오류 텍스트를 보고
    자연히 "정답 아님"으로 판정해(fail-closed) 그 문항만 틀리고, evaluate()/
    갱신 사이클 전체는 죽지 않는다."""
    if not hits:
...
def judge_answer(answer, ex, model=JUDGE_MODEL):
    ...
    judge 호출 실패는 0(fail-closed)으로 처리한다(gate.py의 default_judge_fn과
    동일 철학) — 예외를 던지면 evaluate()가 골드셋 중간에 죽어 갱신 사이클
    전체(promote_if_better)가 멈춘다."""
    prompt = (
...
def judge_quality(answer, ex, model=JUDGE_MODEL):
    ...
    API 호출/파싱 실패는 예외를 던지지 않고 최저점(1)으로 fail-closed 처리한다
    — --qualitative 리포트 한 문항이 깨지는 것과 evaluate() 전체가 죽는 것은
    비용이 다르다."""
    prompt = (
        "You are grading a candidate answer against a reference answer for "
...
def evaluate(
    ...
    — "recall@k"/"correctness" 키와 의미는 promote.py의 회귀 판정이 의존하므로
    절대 바뀌지 않는다.

    qualitative=True면 같은 gen_fn 출력을 재사용해 quality_judge_fn을 추가로
    1회 호출, groundedness/completeness/relevance(1-5 평균) + qualitative_report
    를 옵트인으로 덧붙인다."""
