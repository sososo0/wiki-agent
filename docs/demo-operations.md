...
있다(프로세스 메모리 dict가 아님) — 데모를 여러 워커 프로세스로 띄워도(uvicorn
`--workers N`) 모든 워커가 같은 DB를 보므로 한도가 워커 수만큼 곱해지지 않는다.
버스트 리미터(`_ip_burst_log`)만 예외로 여전히 프로세스 메모리다 — 모든 요청마다
도는 미들웨어라 DB로 옮기면 그 자체가 매 요청에 쓰기를 추가하는 역효과이고,
멀티 워커에서 한도가 조금 느슨해지는 정도라 비용/효과가 안 맞는다고 판단.
