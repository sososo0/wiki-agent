...
| `WIKI_AGENT_DEMO_BURST_LIMIT` / `_BURST_WINDOW_SECONDS` | `8` / `10` | IP 1개가 윈도우 동안 보낼 수 있는 최대 요청 수(`/static/*` 제외 전 경로 적용 — `/graph`/`/history` 등 로컬 연산도 CPU 비용이 있어서. 정적 파일은 그 비용이 없어 그래프/채팅 페이지 로딩 시 스크립트 요청들이 예산을 갉아먹지 않도록 집계에서 뺐다) |
...
| `WIKI_AGENT_DEMO_ADMIN_TOKEN` | (없음) | 설정하면 `/notifications/{id}/read`가 `X-Admin-Token` 헤더로 이 값을 요구(403 없으면 통과). 운영자가 배포 시 설정하고, 자기 브라우저 devtools로 `localStorage.setItem("wiki_agent_admin_token", "...")`를 같은 값으로 넣어둬야 함(입력 UI 없음). |

한도 도달 시 API를 호출하지 않고 고정 문구로 응답. `search_wiki`(로컬 임베딩, 비용
없음)는 한도와 무관하게 항상 수행되어 `retrieval_log`는 계속 쌓인다 — 막히는 건
LLM 호출(`generate`)뿐이라 gap 마이닝 신호는 끊기지 않음.

daily/conv/ip_daily 카운터는 `core/wiki_store.py`의 `llm_call_budget` 테이블에
있다(프로세스 메모리 dict가 아님) — 워커를 여러 개 띄워도(uvicorn `--workers N`)
같은 DB를 보므로 한도가 워커 수만큼 곱해지지 않는다. 버스트 리미터
(`_ip_burst_log`)만 예외로 메모리에 남겨뒀다 — 모든 요청마다 도는 미들웨어라
DB로 옮기면 매 요청에 쓰기가 추가되는데, 멀티 워커에서 한도가 조금 느슨해지는
정도로는 그 비용이 안 맞는다고 판단.
...
**`owner_token` 범위 제한**: `/conversations`/`/history/{conv_id}`는 브라우저
(localStorage)당 발급되는 `owner_token`으로 막혀 있다 — `/chat` 요청마다 같이
보내면 `core/wiki_store.ensure_conversation_owner`가 그 conv_id에 처음 한 번만
기록(이미 있으면 덮어쓰지 않아 소유권 가로채기 방지). `/conversations`는
owner_token이 없으면 빈 목록(fail-closed), `/history/{conv_id}`는 기록된 토큰과
다르면 404. 이 기능 이전에 만들어진 대화는 owner_token이 NULL이라
`/conversations` 목록에는 안 뜨지만 conv_id를 직접 아는 `/history` 조회는
허용된다(레거시 호환).
...
`core/pipeline/dedupe.py`는 기본적으로 chunk_hash exact match로만 "콘텐츠 불변"을
판단해서, 오탈자 수정처럼 의미상 거의 같은 편집도 전체 재큐레이션(LLM 호출)을
부른다. `scripts/ingest_doc.py --near-duplicate-check`(기본 off)를 켜면, chunk_hash가
달라도 기존 엔트리의 영속 임베딩(`core/wiki_store.get_embedding`)과 코사인 유사도가
`--near-duplicate-threshold`(기본 0.97) 이상이면 재큐레이션을 생략한다 — 기존 엔트리에
임베딩이 아직 없으면(검색/그래프를 한 번도 안 거침) 안전하게 기존 동작(재큐레이션)으로
폴백.
