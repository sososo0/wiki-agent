...
## 11. 포트폴리오 서술(README)에 꼭 넣을 것

1. **문제 정의**: 정적 RAG는 시간이 지나면 stale·미응답 증가. → 로그 기반 자가 갱신으로 해결.
2. **아키텍처 다이어그램** (1장)과 데이터 흐름 설명.
3. **데이터 엔지니어링 강조**: 파이프라인 DAG, 스키마, 증분 색인, provenance 설계.
4. **오염 방지 설계**: "왜 에이전트 답을 그냥 넣으면 안 되는가"와 그 해법 → 성숙도 어필.
5. **결과(머니 차트)**: N 사이클에 걸친 지표 향상 + 구체 수치(예: recall@5 0.61→0.79, escalation 18%→7%).
6. **인프라**: 컨테이너화·IaC·관측 구성 간단 명시.
7. **한계와 다음 단계**: 솔직하게(예: 사람 검토 의존 구간, 비용/지연 트레이드오프).

---

## 12. 스트레치 골 (여유 있으면)

- 멀티홉 검색 / GraphRAG로 엔트리 간 관계 활용
- 능동 학습: 모델이 "내가 모르는 영역"을 스스로 질문 생성해 KB 보강
- A/B: 갱신 정책(보수적 vs 공격적)별 온라인 지표 비교
- 비용/지연 SLO 모니터링과 자동 스케일링
