...
# entry_id+version 키 임베딩 캐시(demo/app.py도 동일 패턴) — 안 주면 search_wiki가
# stdio로 오래 사는 이 프로세스에서 매 호출마다 활성 엔트리 전체를 재인코딩한다.
# LRU 메모리 캐시가 1차, 미스 시 DB(wiki_embedding)를 먼저 봐서 프로세스 재시작이나
# demo/app.py와의 공유에도 재인코딩 비용이 들지 않는다.
...
if __name__ == "__main__":
    mcp.run()        # stdio 트랜스포트 (Hermes/Claude Code 등이 프로세스 관리)
